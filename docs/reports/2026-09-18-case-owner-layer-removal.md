...
Date: 2026-09-18
Branch: `feat/eliminate-case-owner-layer` (see `git log` on that branch for the exact commits)
...
this documentation pass), followed by a final review fix wave; the commit list on branch
`feat/eliminate-case-owner-layer` (`git log`) records each step.
...
| Case creator's access | Depended on what was stored in `extra_owners`. `seedOwners` (`createCase`/`quickLog`) stored only the account's real handlers when it had at least one, so the creator was not an owner there; it stored the creator only when the account had no real handler (Direct-only or customerless). The first-quote auto-case stored `extraOwners: []`, so its creator owned it only through the `owner` fallback, which lasted until a handler-add wrote the new handler onto that open case (closed ones were skipped). A creator stored in `extra_owners` stayed there when a handler was later added (the propagation appended to the stored set) and when the case was mapped to a customer; `removeCaseOwner` could remove them. The "creator"/"manual" labels were computed on read, never stored. | An owner only through the live creator fallback, i.e. only while the case's account has zero real handlers. Once the account gains a real handler, or a customerless case is mapped to a customer that has one, the creator drops out of the case's handlers; they keep access only as the assignee, a handler, a tag-matched L3+, or L4+. A confirmed access reduction, see below. |
| Case visibility rule | `caseVisible()` = L4+ OR case-owner (materialized) OR assignee OR `customerAccess==='FULL'`. | `caseVisible()` = L4+ OR `customerAccess==='FULL'` (covers real handler, tag-matched L3+, L4+) OR assignee OR `caseHandlers(...).includes(caller)` (only reachable when there's no real handler, i.e. the narrow creator fallback). Signature changed: `caseVisible()` (and `ensureCanSeeCase()`) now take a required `ownership: AccessOwnership` fourth parameter, which the creator-fallback check needs; internals simplified/reordered in `src/server/auth/access.ts`. |
...
1. **The creator of a case created on a handler-less (Direct-only) or customerless account loses
   their claim to it once that account gains a real handler, or once the customerless case is mapped
   to a customer that has one.** The old model stored the creator in `extra_owners` for exactly these
   cases, and neither a later handler-add (which appended to the stored set) nor mapping (which wrote
   only `customer_id`) removed them. The creator keeps access if they are the assignee (or through
   customer access: a handler, a tag-matched L3+, or L4+). Cases created on an account that already
   had a real handler are unaffected: the old model never stored their creator either. One further
   narrow path: the old first-quote auto-case stored no owners, so its creator owned it through the
   `owner` fallback until the next handler-add reached it while open (never, if it was closed first);
   the new model gives that creator no claim once the account has a real handler.
...
- **Dashboard won-value credit moves retroactively.** The dashboard credits a Won case's value to
  every current handler of its account (`caseHandlers()` in `src/server/dashboard/service.ts`), and
  closed cases are not frozen. Adding or removing a handler therefore moves that account's past
  won-value credit (this month / last 14 days) onto or off that person. The old model credited the
  stored owner set, which handler changes did not rewrite on closed cases.

## Verification

- `npm test`: **42 test files, 760 tests, all passing** (after the final review fix wave; 758 before it).
