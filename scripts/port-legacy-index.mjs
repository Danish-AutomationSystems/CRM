...
// Normalise line endings before anything else touches the text.
//
// Every transform below is an EXACT string match written with \n. On a Windows
// checkout with core.autocrlf=true, git rewrites Index.html with \r\n, every one of
// those matches silently finds nothing, and the generator emits a broken artifact
// while reporting success - the jsArg helper simply never gets injected. That failure
// mode cost a debugging session and is exactly the kind of silent corruption that got
// this artifact frozen in the first place. Normalise on read so the transforms see the
// text they were written against, whatever git did to the working copy.
const source = fs.readFileSync(sourcePath, 'utf8').replace(/\r\n/g, '\n');

/**
 * An exact-match replace that FAILS LOUDLY when its anchor is missing.
 *
 * A silent no-op replace is the single most dangerous operation in this file: the
 * output stays syntactically valid, so every syntax check passes, and the damage only
 * surfaces at runtime. Anything structurally load-bearing must go through this.
 */
function mustReplace(text, find, replacement, label) {
  if (!text.includes(find)) {
    throw new Error(
      `port-legacy-index: anchor for "${label}" not found in Index.html.\n` +
        'The source has drifted from the transform. Fix the anchor rather than ' +
        'letting the generator emit an artifact that is missing this rewrite.'
    );
  }
...
function jsJsonArg(v){ return esc(JSON.stringify(v)); }`,
  'jsArg/jsJsonArg helper injection'
);
...
// Every helper the generator injects must actually be defined in what it emits.
//
// The artifact stays syntactically valid when a helper definition is missing, so
// `new Function(script)` passes and the failure only appears at runtime as
// "<helper> is not defined" the first time a user hits that code path. This checks
// each injected helper is defined wherever it is called.
for (const helper of ['jsArg', 'jsJsonArg', 'setHtml', 'setText']) {
  const called = new RegExp(`\\b${helper}\\(`).test(script);
  const defined = new RegExp(`function ${helper}\\b`).test(script);
  if (called && !defined) {
    throw new Error(
      `port-legacy-index: the generated script calls ${helper}() but never defines it. ` +
        'An injection step matched nothing. Refusing to write a broken artifact.'
    );
  }
}
